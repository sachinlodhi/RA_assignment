...
def freq_graph(rec):
    ...
    for i in attributes_to_plot:
        ...
        plt.savefig(save_dir+'/frequency_graphs/' +i+".svg",format='svg', dpi=50)
...
def scatter_plt(rec):
    ...
    for i, attribute in enumerate(attributes_to_plot):
        ...
        plt.savefig(save_dir + '/scatter_graphs/' + attribute + ".svg",format='svg', dpi=50)
...
def corr_mat_ord(rec):
    ...
    plt.savefig(save_dir + "/heatmaps/" + "ordinal.svg",format='svg', dpi=50)
    # plt.show()
...
def corr_mat_cat(rec, df_chi, df_pVal, df_cramer):
    ...
    for i in dfs:
        ...
        plt.savefig(save_dir + "/heatmaps/" + str(i) +".svg", format='svg', dpi=50)
